export type ValidTags =
  | ReturnType<typeof getGlobalTag>
  | ReturnType<typeof getUserTag>
  | ReturnType<typeof getIdTag>
  | ReturnType<typeof getCenterTag>;

type CACHE_TAG =
  | "users"
  | "studies"
  | "centers"
  | "trials"
  | "crops"
  | "nirsData"
  | "nirModels"
  | "traits"
  | "qualityLabs"
  | "cropCommonNames"
  | "species"
  | "productTypes"
  | "physiologicalStages"
  | "cropTraits"
  | "trialFertilizers"
  | "trialMetadataConfig"
  | "studyMetadataConfig";

export function getGlobalTag(tag: CACHE_TAG) {
  return `global:${tag}` as const;
}

export function getUserTag(tag: CACHE_TAG, userId: number) {
  return `user:${userId}-${tag}` as const;
}

export function getIdTag(tag: CACHE_TAG, id: number | string) {
  return `id:${id}-${tag}` as const;
}

export function getCenterTag(tag: CACHE_TAG, centerId: number | string) {
  return `center:${centerId}-${tag}` as const;
}

export function getTrialTag(tag: CACHE_TAG, trialId: number) {
  return `trial:${trialId}-${tag}` as const;
}

export function getStudyTag(tag: CACHE_TAG, studyId: number) {
  return `study:${studyId}-${tag}` as const;
}

export function getCropTag(tag: CACHE_TAG, cropId: number) {
  return `crop:${cropId}-${tag}` as const;
}

export function getCropTraitTag(tag: CACHE_TAG, cropTraitId: number) {
  return `cropTrait:${cropTraitId}-${tag}` as const;
}
